...
infile = open("C:\\Users\\prabhash.kumar\\OneDrive - Infosys Limited\\Desktop\\infosys\\pythontext\\Offboarding_Report\\body.txt", 'r').readlines()
for line in infile:
    print(line)
# line = line.format(offboarding.bet,offboarding.greter,offboarding.less)
mail.HtmlBody = line.format(fname = offboarding.bet, greater = offboarding.greter,less= offboarding.less, lastlast = offboarding.last_last_month.strftime("%b"), last = offboarding.last_month.strftime("%b") , pivo= offboarding.dff.to_html().replace('<th>','<th style= "background-color:#90CAF9">' ).replace('<tr>','<tr style= "background-color:white">' ))
...
# mail.HtmlBody = line
# mail.HtmlBody = line +offboarding.last_last_month.strftime("%b")+offboarding.last_month.strftime("%b")+offboarding.dff.to_html().replace('<th>','<th style= "background-color:#90CAF9">' ).replace('<tr>','<tr style= "background-color:white">') 


# +offboarding.bet+offboarding.greter+offboarding.less+offboarding.last_last_month.strftime("%b")+offboarding.last_month.strftime("%b")
